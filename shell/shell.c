...
static void cmd_color(char* args) {
    if (!args || args[0] == 0) {
        kprintf("Uso: color [green|red|white|cyan|yellow]\n");
        return;
    }
    if (!strcmp(args, "green")) 
        vga_set_color(0x02, 0x00);
    else if (!strcmp(args, "red")) 
        vga_set_color(0x04, 0x00);
    else if (!strcmp(args, "white")) 
        vga_set_color(0x0F, 0x00);
    else if (!strcmp(args, "cyan")) 
        vga_set_color(0x03, 0x00);
    else if (!strcmp(args, "yellow")) 
        vga_set_color(0x0E, 0x00);
    else 
    kprintf("Color no reconocido: %s\n", args);
}
